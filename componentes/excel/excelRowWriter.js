...
class ExcelRowWriter {
    static async writeRow(worksheet, rowIndex, rowData) {
        ...
        this.writeLine(ws, `${config.ESTADO}`, currentRow, caso.tp, 's');
        ...
        this.writeLine(ws, `${config.ORIGEN}`, currentRow, caso.link, 's');
        this.writeLine(ws, `${config.ESTADO}`, currentRow, caso.tp, 's');

        if (caso.fechaRemate && caso.fechaRemate instanceof Date) {
            ws[`${config.FECHA_REM}` + currentRow] = { v: caso.fechaRemate, t: 'd', z: 'DD/MM/YYYY' };
        }
        this.writeLine(ws, `${config.MARTILLERO}`, currentRow, caso.martillero, 's');

        if (caso.tipoDerecho) {
            this.writeLine(ws, `${config.MARTILLERO}`, currentRow, caso.tipoDerecho, 's');
        } else if (caso.isPaid) {
            this.writeLine(ws, `${config.MARTILLERO}`, currentRow, "(Pagado)", 's');
        } else if (caso.isAvenimiento) {
            this.writeLine(ws, `${config.MARTILLERO}`, currentRow, "(Avenimiento)", 's');
        }

        this.writeLine(ws, `${config.DIRECCION}`, currentRow, caso.unitDireccion, 's');
        this.writeLine(ws, `${config.CAUSA}`, currentRow, caso.causa, 's');
        this.writeLine(ws, `${config.TRIBUNAL}`, currentRow, caso.juzgado, 's');
        this.writeLine(ws, `${config.COMUNA_TRIBUNAL}`, currentRow, this.getComunaJuzgado(caso.juzgado), 's');
        this.writeLine(ws, `${config.COMUNA}`, currentRow, caso.comuna, 's');
        this.writeLine(ws, `${config.ANNO}`, currentRow, caso.anno, 'n');
        this.writeLine(ws, `${config.PARTES}`, currentRow, caso.partes, 's');
        this.writeLine(ws, `${config.DATO}`, currentRow, caso.metros, 's');
        this.writeLine(ws, `${config.VV_O_CUPON}`, currentRow, caso.formatoEntrega, 's');
        this.writeLine(ws, `${config.PORCENTAJE}`, currentRow, caso.porcentaje, 's');
        this.writeLine(ws, `${config.PLAZOVV}`, currentRow, caso.diaEntrega, 's');
        ...
        this.writeLine(ws, `${config.ROL}`, currentRow, caso.unitRol, 's');
        ...
        if (caso.montoMinimo > 100) {
            ...
            this.writeLine(ws, `${config.PRECIO_MINIMO2}`, currentRow, caso.moneda, 's');
        }
        if (caso.montoMinimo2) {
            this.writeLine(ws, `${config.PRECIO_MINIMO2}`, currentRow, caso.montoMinimo2, 'n');
        }
        ...
        this.writeLine(ws, `${config.ESTADO_CIVIL}`, currentRow, caso.estadoCivil, "s");
