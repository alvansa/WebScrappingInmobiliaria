...
class ConsultaCausaPjud {
    constructor(browser, window, caso, mainWindow) {
        ...
        this.caso = caso;
        this.link =
            "https://oficinajudicialvirtual.pjud.cl/includes/sesion-consultaunificada.php";
        this.page = null;
        this.downloadPath = path.join(
            os.homedir(),
            "Documents",
            "infoRemates/pdfDownload",
        );
        this.dirPath = "";
        this.pdfPath = "";
    ...
    async getConsulta() {
        logger.warn(`Iniciando la consulta con el archivo de refactorización`);
        let lineaAnterior = "";
        let result = false;
    ...
    async loadConfig() {
        ...
        try {
            ...
            logger.error(
                `Error parsing USER_AGENTS from .env, using default agents: ${error.message}`,
            );
    ...
    async procesarCaso(lineaAnterior) {
        ...
        try {
            // TODO: Boton de hacer click en la busqueda
    ...
    async getPartesCaso() {
        let partes = null;
        try {
            partes = await this.page.$eval(
                "#dtaTableDetalle tbody tr:first-child",
                (row) => {
                    const cells = row.querySelectorAll("td");
                    const caratulado = cells[3]
        ...
        try {
            ...
            await this.page.waitForFunction(
                async (lineaAnterior) => {
                    ...
                    const lineaActual = document.querySelector(
                        "#dtaTableDetalle tbody tr:first-child",
                    );
                    if (lineaActual) {
                        const cells = lineaActual.querySelectorAll("td");
                        const newContent = Array.from(cells)
    ...
    async getPrimeraLinea() {
        const primeraLinea = await this.page.$eval(
            "#dtaTableDetalle tbody tr:first-child",
            (row) => {
                const cells = row.querySelectorAll("td");
    ...
    async setValoresIncialesBusquedaCausa() {
        ...
        if (!(await this.configurateCorte(valores.corte))) {
            return false;
        }

        if (!(await this.configurateTribunal2(valores.juzgado))) {
            return false;
        }

        if (!(await this.configurateCausa(valores.causa))) {
            return false;
        }

        if (!(await this.configurateAnno(valores.anno))) {
            return false;
        }
        ...
        try {
            ...
            const value = await this.page.evaluate((nombreTribunal) => {
                ...
                const options = Array.from(
                    document.querySelectorAll("#conTribunal option"),
                );
    ...
    async downloadDemanda() {
        const linkBaseDemanda =
            "https://oficinajudicialvirtual.pjud.cl/ADIR_871/civil/documentos/docu.php?valorEncTxtDmda=";
        try {
    ...
    async checkIfCaseIsConcluded() {
        try {
            await this.page.waitForSelector(".table-titulos", {
                timeout: 5000,
            });
            ...
            console.error(
                "Error al verificar si el caso está concluido:",
                error.message,
            );
    ...
    async searchButtonAuction() {
        try {
            ...
            console.error(
                "Enlace al buscar y presionar el enlace: ",
                error.message,
            );
            return false;
        }
    ...
    async selectCuaderno() {
        ...
        try {
            ...
            const optionToSelect = options.find((option) =>
                option.text.includes(targetOptionText),
            );
            ...
            const cuadernoValue = await this.page.$eval(
                "#selCuaderno",
                (el) => el.value,
            );
            if (optionToSelect) {
            ...
            console.error(
                "Error al esperar el selector de cuaderno:",
                error.message,
            );
            return false;
        }
    ...
    async downloadPdfFile() {
        ...
        try {
            const tableOfPdf =
                "#modalAnexoSolicitudCivil > div > div > div.modal-body > div > div > div > table > tbody tr";
            const rows = await this.page.$$(tableOfPdf);
            for (let row of rows) {
                //TODO: Eliminar doc y fecha ya que nunca son necesarios
    ...
    async downloadPdfFromUrl(url) {
        ...
        try {
            ...
            //TODO: Hacer una funcion para crear la carpeta y el nombre del pdf, ya que se repite en varias partes del codigo
            ...
            pdfPage.on("request", (req) => {
                if (req.url() === url) {
                    ...
                    https
                        ...
                        .on("error", (err) => {
                            logger.error(`Error en la petición HTTPS: ${err.message}`);
                        });
                }
            });
    ...
    //                     '.loading, .progress, [aria-busy="true"]',
    //                 );
    ...
    //         }
    //     } catch (error) {
    //         logger.error(`Error al descargar el PDF: ${error.message}`);
    //         return false;
    //     } finally {
    //         if (pdfPage) {
    //             pdfPage.close();
    //         }
    //         if (pdfWindow && !pdfWindow.isDestroyed()) {
    //             logger.info("Cerrando ventana del PDF");
    //             pdfWindow.destroy();
    //         }
    //     }
    // }
    ...
    //             if (stats.size < 1024) {
    //                 // PDF muy pequeño probablemente está corrupto
    //                 throw new Error(
    //                     "El archivo PDF es demasiado pequeño y probablemente está incompleto",
    //                 );
    //             }
    ...
    //                 throw new Error(
    //                     "El archivo PDF no termina correctamente (falta %%EOF)",
    //                 );
    //             }
    ...
    async configurateCorte(valorCorte) {
        try {
            ...
            const valorCortePage = await this.page.$eval(
                "#conCorte",
                (el) => el.value,
            );
    ...
    async configurateTribunal(valorJuzgado) {
        try {
            ...
            const tribunalValue = await this.page.$eval(
                "#conTribunal",
                (el) => el.value,
            );
    ...
    async configurateCausa(valorCausa) {
        try {
            ...
            await this.page.type("#conRolCausa", valorCausa, {
                delay: Math.random() * 45,
            });
            const rolValue = await this.page.$eval(
                "#conRolCausa",
                (el) => el.value,
            );
    ...
    async configurateAnno(valorAnno) {
        try {
            ...
            await this.page.type("#conEraCausa", valorAnno, {
                delay: Math.random() * 45,
            });
            const annoValue = await this.page.$eval(
                "#conEraCausa",
                (el) => el.value,
            );
    ...
    async deleteDirectory() {
        try {
            ...
            console.error(
                `No se pudo eliminar el directorio ${this.dirPath}:`,
                error.message,
            );
            throw error;
        }
